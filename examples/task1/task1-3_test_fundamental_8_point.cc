...
FundamentalMatrix fundamental_8_point(math::Matrix<double, 3, 8> const &points1, math::Matrix<double, 3, 8> const &points2)
{
    ...
    // u'u F11 + v'u F12 + u F13 +
    // u'v F21 + v'v F22 + v F23 +
    // u' F31  + v' F32  + F33 = 0
    // points1 = [u, v, 1]
    // points2 = [u', v', 1]
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        A(i, 1) = points1(0, i) * points2(1, i);
        A(i, 2) = points1(0, i);
        A(i, 3) = points1(1, i) * points2(0, i);
    ...
    F(2, 2) = f(8);
    // SVD F
    math::Matrix<double, 3, 3> U, S, Vf;
    math::matrix_svd<double, 3, 3>(F, &U, &S, &Vf);
    std::cout << "Singular values of F: " << S << std::endl;
    S(2, 2) = 0;
    F = U * S * Vf.transpose();
    return F;
}
...
int main(int argc, char *argv[])
{
    ...
    // 输出基础矩阵是正确答案的转置，可能因为真确答案是列优先的我写的行优先
